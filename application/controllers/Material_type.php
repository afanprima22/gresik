...
class Material_type extends MY_Controller {
	...
	public function view(){
		...
		$data = array(
			...
			'title_page' 	=> 'Setup Data / Type Material',
			'title' 		=> 'Kelolah Data',
			'c'				=> $c
			);

		$this->open_page('material_type_v', $data);
	}
	...
	public function load_data(){
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->material_type_id>0) {
					$response['data'][] = array(
						$val->material_type_name,
						'<button class="btn btn-primary btn-xs" type="button" onclick="edit_data('.$val->material_type_id.'),reset()" '.$u.'><i class="glyphicon glyphicon-edit"></i></button>&nbsp;&nbsp;<button class="btn btn-danger btn-xs" type="button" onclick="delete_data('.$val->material_type_id.')" '.$d.'><i class="glyphicon glyphicon-trash"></i></button>'
					);
					$no++;	
				}
			}
	...
	function general_post_data(){
		$data = array(
			'material_type_name' 		=> $this->input->post('i_name', TRUE),
			);
