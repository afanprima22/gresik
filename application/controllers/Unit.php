...
class Unit extends MY_Controller {
	...
	public function __construct() {
        ...
        $akses = $this->g_mod->get_user_acces($this->user_id,95);
	...
	public function view(){
		...
		$data = array(
			...
			'title_page' 	=> 'Master Data / Unit',
			'title' 		=> 'Kelolah Data',
			'c'				=> $c
			);

		$this->open_page('unit_v', $data);
	}
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->unit_id>0) {
					$response['data'][] = array(
						$val->unit_name,
						'<button class="btn btn-primary btn-xs" type="button" onclick="edit_data('.$val->unit_id.'),reset()" '.$u.'><i class="glyphicon glyphicon-edit"></i></button>&nbsp;&nbsp;<button class="btn btn-danger btn-xs" type="button" onclick="delete_data('.$val->unit_id.')" '.$d.'><i class="glyphicon glyphicon-trash"></i></button>'
					);
					$no++;	
				}
			}
	...
	function general_post_data(){
		$data = array(
			'unit_name' 		=> $this->input->post('i_name', TRUE),
			);
