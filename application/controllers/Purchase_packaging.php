...
class Purchase_packaging extends MY_Controller {
	...
	public function __construct() {
        ...
        $akses = $this->g_mod->get_user_acces($this->user_id,93);
	...
	public function view(){
		...
		$data = array(
			...
			'title_page' 	=> 'Transaction / Pembelian / Bahan Packaging',
			'title' 		=> 'Kelolah Data',
			'c'				=> $c
			);

		$this->open_page('purchase_packaging_v', $data);
	}
	...
	public function load_data_detail($id){
		...
		$select = 'a.*,c.item_name as item_name,g.item_name as item_half_name,b.item_detail_color as item_detail_color,f.item_detail_color as item_half_detail_color,d.material_name,e.sperpart_name,h.package_name';
		//LIMIT
		$limit = array(
			'start'  => $this->input->get('start'),
			'finish' => $this->input->get('length')
		...
		//WHERE LIKE
		$where_like['data'][] = array(
			'column' => 'h.package_name,c.item_name,b.item_detail_color,g.item_name,f.item_detail_color,d.material_name,e.sperpart_name,h.package_name',
			'param'	 => $this->input->get('search[value]')
		);
		//ORDER
		$index_order = $this->input->get('order[0][column]');
		...
		$join['data'][] = array(
			'table' => 'packages h',
			'join'	=> 'h.package_id=a.purchase_detail_data_id and purchase_detail_type = 6',
			'type'	=> 'left'
		);
			...
			foreach ($query->result() as $val) {
				if ($val->purchase_detail_id>0) {
					...
					if ($val->purchase_detail_type == 1 || $val->purchase_detail_type == 2) {
						...
						$type = 'Sperpart';
					}elseif ($val->purchase_detail_type == 6) {
						$ket = $val->package_name;
	...
	public function load_data_where_detail(){
		$select = 'a.*,c.item_name as item_so_name,c.item_id as item_so_id,g.item_name as item_half_name,g.item_id as item_half_id,b.item_detail_color as item_so_detail_color,f.item_detail_color as item_half_detail_color,d.material_name,e.sperpart_name,package_name';
		$tbl = 'purchase_details a';
		//WHERE
		$where['data'][] = array(
	...
	public function action_data_detail($id2){
		$id = $this->input->post('i_detail_id');
		
		if (strlen($id)>0) {
			//UPDATE
			$data = $this->general_post_data_detail($id2);
			//WHERE
			$where['data'][] = array(
				'column' => 'purchase_detail_id',
				'param'	 => $id
	...
	public function delete_data(){
		...
		$delete2 = $this->g_mod->delete_data_table('purchase_details', $where);
		if($delete->status) {
			$response['status'] = '200';
			$response['alert'] = '3';
		} else {
	...
	function general_post_data_detail($id2){
		...
		$packaging 	= $this->input->post('i_packaging', TRUE);
	...
	public function load_data_select_purchase_packaging(){
		//WHERE LIKE
		$where_like['data'][] = array(
			'column' => 'purchase_code',
