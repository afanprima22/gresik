...
class Receipt_bahan_packaging extends MY_Controller {
	...
	public function __construct() {
        ...
        $akses = $this->g_mod->get_user_acces($this->user_id,94);
	...
	public function view(){
		...
		$data = array(
			...
			'title_page' 	=> 'Transaction / Penerimaan Bahan Packaging',
			'title' 		=> 'Kelolah Data',
			'c'				=> $c
			);

		$this->open_page('receipt_bahan_packaging_v', $data);
	}
	...
	public function load_data_detail($id){
		...
		$select = 'a.*,i.package_name,c.item_name as item_name,g.item_name as item_half_name,b.item_detail_color as item_detail_color,f.item_detail_color as item_half_detail_color,d.material_name,e.sperpart_name,h.receipt_detail_sj,h.receipt_detail_cancel,h.receipt_detail_come,h.receipt_detail_id';
		//LIMIT
		$limit = array(
			'start'  => $this->input->get('start'),
			'finish' => $this->input->get('length')
		...
		//WHERE LIKE
		$where_like['data'][] = array(
			'column' => 'i.package_name,c.item_name,b.item_detail_color,g.item_name,f.item_detail_color,d.material_name,e.sperpart_name',
			'param'	 => $this->input->get('search[value]')
		);
		//ORDER
		$index_order = $this->input->get('order[0][column]');
		...
		$join['data'][] = array(
			'table' => 'packages i',
			'join'	=> 'i.package_id=a.purchase_detail_data_id and purchase_detail_type = 6',
			'type'	=> 'left'
		);
	...
	public function delete_data(){
		...
		$delete2 = $this->g_mod->delete_data_table('receipt_details', $where);
		if($delete->status) {
			$response['status'] = '200';
			$response['alert'] = '3';
		} else {
