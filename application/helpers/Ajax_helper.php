...
if (! function_exists('format_code_desc')) {
	function format_code_desc($table, $column, $code_length = 4,$prefix = '', $condition = '', $separator = '')
	{
		$number = 0;
		$code = array();
		...
		$ci = & get_instance();
		$ci->db->select($column);
		if($prefix)$ci->db->like($column, $prefix, 'before');
		if($condition)$ci->db->where($condition);
		$ci->db->order_by($column, 'desc');
		$query = $ci->db->get($table, 1);
...
if (! function_exists('format_code_khusus')) {
	function format_code_khusus($table, $column,$prefix = '', $code_length = 4,$prefix2 = '', $condition = '', $separator = '')
	{
		$number = 0;
		$code = array();
...
if ( ! function_exists('nama_bulan'))
{
    ...
    {
		$bulan = array(1=>'Januari',2=>'Februari',3=>'Maret',4=>'April',5=>'Mei',6=>'Juni',7=>'Juli',8=>'Agustus',9=>'September',10=>'Oktober',11=>'Nonember',12=>'Desember');
...
if ( ! function_exists('format_time_12'))
{
...
if ( ! function_exists('format_time_24'))
{
...
if ( ! function_exists('format_date_month_mid'))
{
...
if ( ! function_exists('format_date_month_mid2'))
{
