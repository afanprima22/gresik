                      ...
                      <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <div class="input-group date">
                              ...
                              <input type="text" class="form-control pull-right" id="datepicker" name="i_date_recept_packaging" placeholder="Tanggal Penerimaan" value="" required="required">
                            </div>
                          </div>
...
<script type="text/javascript">
    ...
    function search_data() { 
        $('#table1').DataTable({
            ...
            "serverSide": true,
            ajax: {
              url: '<?php echo base_url();?>recept_packaging/load_data/'
            },
            "columns": [
              {"name": "packaging_id"},
              {"name": "recept_packaging_date"},
              {"name": "recept_packaging_code"},
    ...
    function search_data_detail(id) { 
        $('#table3').DataTable({
            ...
            "serverSide": true,
            ajax: {
              url: '<?php echo base_url();?>recept_packaging/load_data_detail/'+id,
            },
            
          
            "columns": [
              {"name": "recept_packaging_detail_id"},
    ...
    function search_data_packaging_detail_item(id) { 
      if (!id) {
        ...
        $('#table3').DataTable({
            ...
            "serverSide": true,
            ajax: {
              url: '<?php echo base_url();?>Recept_packaging/load_data_detail_item/'+id
            },
            "columns": [
    ...
    function edit_data(id) {
        $.ajax({
          type : "GET",
          url  : '<?php echo base_url();?>recept_packaging/load_data_where/',
          data : "id="+id,
          dataType : "json",
          success:function(data){
            for(var i=0; i<data.val.length;i++){
              document.getElementById("i_id").value             = data.val[i].recept_packaging_id;
              document.getElementById("datepicker").value           = data.val[i].recept_packaging_date;

              $("#i_code").append('<option value="'+data.val[i].packaging_id+'" selected>'+data.val[i].packaging_code+'</option>');
              search_data_detail(data.val[i].recept_packaging_id);
    ...
    function action_data(){
      ...
      save_detail();
        $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>recept_packaging/action_data/',
    ...
    function save_detail(){
        ...
        $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Recept_packaging/action_data_detail/',
          data : $( "#formall" ).serialize(),
          dataType : "json",
          success:function(data){
            if(data.status=='200'){
    ...
    function save_stock(){
        ...
        $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Recept_packaging/action_data_stock/'+id,
          data : $( "#formall" ).serialize(),
          dataType : "json",
          success:function(data){
            if(data.status=='200'){
    ...
    function delete_data(id) {
        ...
        if(a==true){
            $.ajax({
                url: '<?php echo base_url();?>recept_packaging/delete_data',
                data: 'id='+id,
                type: 'POST',
                dataType: 'json',
                success: function (data) {
    ...
    function delete_data_detail(id_detail) {
       ...
       if(a==true){
           $.ajax({
               url: '<?php echo base_url();?>recept_packaging/delete_data_detail',
               data: 'id='+id_detail,
               type: 'POST',
               dataType: 'json',
               success: function (data) {
    ...
    function hapus() {
      $.ajax({
        url: '<?php echo base_url();?>recept_packaging/hapus',
        type: 'POST',
        dataType: 'json',
        success: function (data) {
          if (data.status=='200') {
    ...
    function reset2(){
      ...
      $('input[name="i_date_recept_packaging"]').val("");
      $('input[name="i_id"]').val("");
