                ...
                <form id="formall" role="form" action="" method="post" enctype="multipart/form-data" onkeypress="return event.keyCode != 13;">
                    <div class="box-content">
                      <div class="row">
                        ...
                        <div class="col-md-12">
                          <div id="print_order_kongsi" style="display: none;float: left;">
                            <button type="button"  onclick="print_order_kongsi()" class="btn btn-info"><i class="glyphicon glyphicon-print"></i> order_kongsi</button>&nbsp;
                          </div>
...
<script type="text/javascript">
    ...
    function search_data() { 
        $('#table1').DataTable({
            ...
            "serverSide": true,
            ajax: {
              url: '<?php echo base_url();?>Order_kongsi/load_data/'
            },
            "columns": [
              {"name": "order_kongsi_code"},
              {"name": "kongsi_name"},
    ...
    function search_data_detail(id) { 
        $('#table2').DataTable({
            ...
            "serverSide": true,
            ajax: {
              url: '<?php echo base_url();?>Order_kongsi/load_data_detail/'+id
            },
            "columns": [
              {"name": "item_name"},
              ...
              {"name": "order_kongsi_detail_discount"},
              {"name": "order_kongsi_detail_total","orderable": false,"searchable": false},
              {"name": "action","orderable": false,"searchable": false, "className": "text-center"}
            ],
            "order": [
              [0, 'asc']
    ...
    function action_data(){
        $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Order_kongsi/action_data/',
    ...
    function delete_data(id) {
        ...
        if(a==true){
            $.ajax({
                url: '<?php echo base_url();?>Order_kongsi/delete_data',
                data: 'id='+id,
                type: 'POST',
                dataType: 'json',
                success: function (data) {
    ...
    function edit_data(id) {
        $.ajax({
          type : "GET",
          url  : '<?php echo base_url();?>Order_kongsi/load_data_where/',
          data : "id="+id,
          dataType : "json",
          success:function(data){
            reset2();
            for(var i=0; i<data.val.length;i++){
              document.getElementById("i_id").value             = data.val[i].order_kongsi_id;
              document.getElementById("datepicker").value           = data.val[i].order_kongsi_date;
              document.getElementById("i_discount").value             = data.val[i].order_kongsi_discount;
              ...
              search_data_detail(data.val[i].order_kongsi_id);

               /*document.getElementById('print_sj').style.display = 'block';
              if (data.val[i].order_kongsi_sj == 1) {
                document.getElementById('print_order_kongsi').style.display = 'block';
              }*/
            }
          }
        });
    ...
    function select_list_kongsi() {
      ...
      function get_memo() {
        ...
        var spg_id = document.getElementById("i_spg").value;
        var kongsi_id = document.getElementById("i_kongsi").value;
        ...
        $('#selectmemo').select2({
          ...
          allowClear: true,
          ajax: {
            url: '<?php echo base_url();?>Order_kongsi/load_data_select_memo/',
            dataType: 'json',
            delay: 100,
            cache: true,
            data: function (params) {
      ...
      function reset2(){
        ...
        document.getElementById('print_order_kongsi').style.display = 'none';
      ...
      function save_detail(){
        ...
        $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Order_kongsi/action_data_detail/',
          data : $( "#formall" ).serialize(),
          dataType : "json",
          success:function(data){
            if(data.status=='200'){
      ...
      function edit_data_detail(id){
        $.ajax({
          type : "GET",
          url  : '<?php echo base_url();?>Order_kongsi/load_data_where_detail/',
          data : "id="+id,
          dataType : "json",
          success:function(data){
            for(var i=0; i<data.val.length;i++){
              $('input[name="i_detail_id"]').val(data.val[i].order_kongsi_detail_id);
              $('input[name="i_detail_qty"]').val(data.val[i].order_kongsi_detail_qty);
              $('input[name="i_detail_price"]').val(data.val[i].order_kongsi_detail_price);
              $('input[name="i_detail_discount"]').val(data.val[i].order_kongsi_detail_discount);
      ...
      function delete_data_detail(id_detail) {
        ...
        if(a==true){
            $.ajax({
                url: '<?php echo base_url();?>Order_kongsi/delete_data_detail',
                data: 'id='+id_detail,
                type: 'POST',
                dataType: 'json',
                success: function (data) {
    ...
    function get_price(){
      ...
      $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Order_kongsi/get_price/',
          data : $( "#formall" ).serialize(),
          dataType : "json",
          success:function(data){
    ...
    function get_grand_total(id){
        ...
        /*$.ajax({
          type : "GET",
          url  : '<?php echo base_url();?>Order_kongsi/get_discount/',
          data : "id="+id,
          dataType : "json",
          success:function(data){
            var discount_name = "";
        ...
        //$("#detail_discount").load("<?= base_url()?>Order_kongsi/get_discount/"+id);

        $.ajax({
          type : "GET",
          url  : '<?php echo base_url();?>Order_kongsi/get_grand_total/',
          data : "id="+id,
          dataType : "json",
          success:function(data){
    ...
    function print_order_kongsi(){
      var id = document.getElementById("i_id").value;

      window.open('<?php echo base_url();?>Order_kongsi/print_order_kongsi_pdf?id='+id);
      //alert(id)
    }
    ...
    function print_sj(){
      ...
      window.open('<?php echo base_url();?>Order_kongsi/print_surat_jalan_pdf?id='+id);
    ...
    function update_discount(value){
      ...
      $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Order_kongsi/update_discount/',
